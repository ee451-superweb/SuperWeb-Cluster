...
belong to a requested workload mode such as ``small``, ``medium``, or ``full``.
"""
...
def uses_medium_dataset(mode: str) -> bool:
    """Return whether the requested mode should include the medium dataset.

    Args:
        mode: Requested workload mode string.

    Returns:
        ``True`` when the medium dataset should be generated or benchmarked.
    """

    return mode == WORKLOAD_MODE_MEDIUM
