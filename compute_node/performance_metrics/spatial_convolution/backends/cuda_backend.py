"""CUDA backend for the spatial-convolution benchmark.
...
from app.compute_resource_policy import (
    build_spatial_cuda_output_channel_batch_candidates,
    resolve_scaled_spatial_cuda_output_channel_batch,
)
...
from compute_node.performance_metrics.spatial_convolution.scoring import (
    linear_time_score,
)
...
SPATIAL_CUDA_OUTPUT_CHANNEL_BATCH_OVERRIDE: tuple[int, ...] | None = None
SPATIAL_CUDA_OUTPUT_CHANNEL_BATCH_SCALE = DEFAULT_SPATIAL_CUDA_OUTPUT_CHANNEL_BATCH_SCALE
...
def _candidate_output_channel_batches(spec: BenchmarkSpec | None = None) -> list[int]:
    ...
    """
    if SPATIAL_CUDA_OUTPUT_CHANNEL_BATCH_OVERRIDE is not None:
        return [int(value) for value in SPATIAL_CUDA_OUTPUT_CHANNEL_BATCH_OVERRIDE]
    output_channels = 64 if spec is None else spec.c_out
    return build_spatial_cuda_output_channel_batch_candidates(output_channels)
...
class CudaBackend:
    ...
    def diagnostic_context(self, spec: BenchmarkSpec | None = None) -> dict[str, object]:
        ...
        return {
            ...
            "output_channel_batch_scale": SPATIAL_CUDA_OUTPUT_CHANNEL_BATCH_SCALE,
            "cooldown_ms": SPATIAL_CUDA_COOLDOWN_MS,
        }
    ...
    def run(
        ...
        *,
        ...
        selected_measurement_batch = resolve_scaled_spatial_cuda_output_channel_batch(
            selected_autotune_batch,
            measurement_spec.c_out,
            scale=SPATIAL_CUDA_OUTPUT_CHANNEL_BATCH_SCALE,
        )
        ...
        if selected_measurement_batch != selected_autotune_batch:
            notes.append(
                ...
                f"using runtime batch scale {SPATIAL_CUDA_OUTPUT_CHANNEL_BATCH_SCALE:.3f}."
            )
        ...
        autotune_config = {
            ...
            "output_channel_batch_scale": SPATIAL_CUDA_OUTPUT_CHANNEL_BATCH_SCALE,
            "cooldown_ms": float(autotune_metrics.get("cooldown_ms", SPATIAL_CUDA_COOLDOWN_MS)),
        ...
        measurement_config = {
            ...
            "output_channel_batch_scale": SPATIAL_CUDA_OUTPUT_CHANNEL_BATCH_SCALE,
            "cooldown_ms": float(measurement_metrics.get("cooldown_ms", SPATIAL_CUDA_COOLDOWN_MS)),
            "autotune_repeats": int(autotune_metrics["autotune_repeats"]),
    ...
    def _compile_windows_runner(self, capability: str | None) -> None:
        ...
        compile_script_path = CUDA_BUILD_DIR / "build_spatial_conv_cuda_runner.cmd"
