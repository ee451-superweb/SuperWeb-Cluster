"""Metal backend for the spatial-convolution benchmark on macOS.

Use this module to probe Metal availability, run the native Metal benchmark
runner, and convert runner output into benchmark models.
"""
...
def _candidate_block_sizes() -> list[int]:
    """Return Metal block-size candidates for autotune."""
    return [32, 64, 128, 256, 512, 1024]

def _candidate_tile_sizes() -> list[int]:
    """Return Metal tile-size candidates for autotune."""
    return [1, 2, 4, 8, 16]

class MetalBackend:
    """Run the native spatial Metal benchmark backend on macOS."""
    ...
    def probe(self) -> tuple[bool, str]:
        """Check whether this machine can run the spatial Metal backend."""
    ...
    def run(
        ...
        *,
        ...
        """Run autotune plus final measurement for the Metal backend."""
