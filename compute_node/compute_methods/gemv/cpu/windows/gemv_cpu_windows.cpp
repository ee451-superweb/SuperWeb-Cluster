...
struct TrialRecord {
    ...
    int candidate_index = 0;            // position in the swept candidate list
    int candidate_total = 0;            // total candidates for this phase
...
// Parse the small command-line surface used by both benchmark mode and task mode.
Options parse_args(int argc, char** argv) {
    Options options;
    int index = 1;
    while (index < argc) {
        const std::string key = argv[index];
        // --verbose is a boolean flag with no value — handle it before the
        // value-consuming branches.
        if (key == "--verbose") {
...
int main(int argc, char** argv) {
    try {
        ...
        // Memory-traffic model (FC-layer analogy): the matrix A plays the role
        // of "weights", the vector x plays the role of "input activations", and
        // y is the output. Compulsory traffic is the one-touch lower bound.
        ...
        auto emit_verbose_trial = [&](const TrialRecord& tr, int global_index, int global_total) {
            ...
            fprintf(stderr,
                    "[gemv cpu (windows) %s %d/%d] candidate=%d/%d (trial %d/%d) "
        ...
        if (options.task_mode) {
            if (options.verbose) {
                fprintf(stderr,
                        "[gemv cpu (windows) plan] phase=task_execution "
        ...
        if (options.verbose) {
            fprintf(stderr,
                    "[gemv cpu (windows) plan] phase=autotune worker_candidates=%zu "
        ...
        for (const int requested_workers : options.worker_candidates) {
            for (const int tile_size : options.tile_sizes) {
                ...
                // measure_config returns the per-repeat AVERAGE latency; we
                // record one TrialRecord per candidate, treating the averaged
                // latency as both compute and total wall time (no H2D/D2H on CPU).
                TrialRecord tr;
                tr.phase = "autotune";
        ...
        if (options.verbose) {
            fprintf(stderr,
                    "[gemv cpu (windows) plan] phase=measurement "
