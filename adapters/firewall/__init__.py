...
@trace_function
def ensure_rules(platform_info: PlatformInfo, discovery_port: int, data_plane_port: int = 0) -> FirewallStatus:
    ...
    if platform_info.platform_name == "windows":
        ...
        return windows_ensure_rules(
            discovery_port=discovery_port,
            is_admin_user=platform_info.is_admin,
...
@trace_function
def cleanup_rules(platform_info: PlatformInfo, discovery_port: int, data_plane_port: int = 0) -> FirewallStatus:
    """Remove kickoff firewall rules when supported."""

    if platform_info.platform_name == "windows":
        from .windows import cleanup_rules as windows_cleanup_rules

        return windows_cleanup_rules(
            discovery_port=discovery_port,
            is_admin_user=platform_info.is_admin,
            data_plane_port=data_plane_port,
        )
