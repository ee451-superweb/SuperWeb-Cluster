...
from app.constants import DX12_BACKEND_DISABLED_REASON, METHOD_SPATIAL_CONVOLUTION
from compute_node.compute_methods.spatial_convolution import executor as spatial_executor
from compute_node.performance_metrics.spatial_convolution.models import BenchmarkSpec as SpatialBenchmarkSpec
...
class TaskExecutorCommandTests(unittest.TestCase):
    ...
    def test_spatial_cuda_task_command_uses_batch_and_cooldown_flags(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (temp_root / "test_input.bin").write_bytes(b"\0")
            (temp_root / "runtime_input.bin").write_bytes(b"\0")
            ...
            executor = spatial_executor.SpatialConvolutionTaskExecutor(
            ...
            with (
                ...
                mock.patch.object(spatial_executor, "SPATIAL_CUDA_COOLDOWN_MS", 1.5),
                mock.patch.object(spatial_executor, "_runner_path", return_value=fake_runner),
                mock.patch("compute_node.compute_methods.spatial_convolution.executor.subprocess.run", side_effect=fake_run),
            ):
    ...
    def test_spatial_cpu_task_command_uses_global_worker_cap(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with (
                ...
                mock.patch.object(spatial_executor, "_runner_path", return_value=fake_runner),
                mock.patch(
                    "compute_node.compute_methods.spatial_convolution.executor.resolve_capped_cpu_worker_count",
                    return_value=9,
                ),
                mock.patch("compute_node.compute_methods.spatial_convolution.executor.subprocess.run", side_effect=fake_run),
            ):
